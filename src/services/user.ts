...
export const UserService = {
  registerParticipant: async (entity: IRegisterParticipant) => {
    ...
    try {
      ...
      let voucherURL = null;
      if (
        entity.typePayment === METHOD_PAYMENT.TRANSFER &&
        entity.voucherBase64
      )
  ...
  registerNewAdmin: async (email: string) => {
    // Verificar si el administrador ya está registrado
    const existingAdmin = await UserModel.findOne({ role: ROLES.ADMINISTRATOR });
    if (existingAdmin) {
    ...
    // Enviar la contraseña por correo electrónico al nuevo administrador
    enviarEmail(email, 'Contraseña para el nuevo administrador', `Su contraseña es: ${password}`);
